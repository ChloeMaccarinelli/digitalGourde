...
export class PortefeuillePrincipalBrhComponent implements OnInit {
  ...
  confirmAddMonnie(contact) {
    this.service.makeRequest(apiUrl.issueDHTG, { password: this.password, id: 209, montant: this.quantite }).
      subscribe(res => {
        console.log('yes');
        this.getSoldeBRH();
      ...
      res => {
        for (const portefeuille of res) {
          ...
          this.service.makeRequest(apiUrl.transferTo, {
            id: 209, password: 'aPassword', clePubDestinataire: this.portefeuilleDestinataire.ClePub
